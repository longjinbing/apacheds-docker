...
The project sources can be found on [GitHub](https://github.com/microservice/apacheds-docker). The Docker image on [Docker Hub](https://registry.hub.docker.com/u/microservice/apacheds/).


## Build

    git clone https://github.com/microservice/apacheds-docker.git
    docker build -t microservice/apacheds:2.0.0.AM26 apacheds-docker
...
The container can be started issuing the following command:
    ...
    docker run --name ldap -d -p 389:10389 microservice/apacheds


## Usage

You can manage the ldap server with the admin user *uid=admin,ou=system* and the default password *secret*. The *default* instance comes with a pre-configured partition *dc=microservice,dc=site*.
...
To customize the existing configuration with a different root DC you need to find and replace a number of strings within `ome.ldif`, `instance/config.ldif` and `instance/ads-contextentry.decoded`. Specifically find and replace `dc=site`, `dc: org`, `microservice.site` and `microservice`.

For a custom root dc of `example.com`:

```shell
$ sed -i 's/microservice/example/g' ome.ldif ./instance/config.ldif ./instance/ads-contextentry.decoded
$ sed -i 's/dc=site/dc=com/g' ome.ldif ./instance/config.ldif ./instance/ads-contextentry.decoded
